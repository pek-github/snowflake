...
(defonce bit-lengths {:timestamp 41 :cluster 5 :machine 5 :sequence-number 12})

(defonce bit-positions 
  {:timestamp (+ (:cluster bit-lengths) (:machine bit-lengths) (:sequence-number bit-lengths))
...
(defonce epoch (chronos/to-long "2016-01-01"))
    ...
    (let [current-timestamp (now)]
      (if (= current-timestamp (ensure real-timestamp))
        (if (> (ensure sequence-number) max-sequence-number)
          (do ; "The sequence numbers have been exhausted for this millisecond"
          ...
          ; "We need the next sequence number for this millisecond"
          (let [sequence-value (ensure sequence-number)]
            (alter sequence-number inc)
            (create-id current-timestamp sequence-value)))
            
        (do ; "We need to reset the sequence numbers and update for the new millisecond"
          (ref-set sequence-number 1)
